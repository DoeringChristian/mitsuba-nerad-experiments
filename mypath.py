...
class MyPathIntegrator(mi.SamplingIntegrator):
    ...
    def sample(
        ...
        valid_ray = mi.Mask(
            (~mi.Bool(self.hide_emitters)) & dr.neq(scene.environment(), None)
        )
        ...
        while loop(active):
            ...
            active = (
                active_next & (~rr_active | rr_continue) & dr.neq(throughput_max, 0)
            )
    ...
    def emitter_hit(
        self, scene, throughput, prev_si, prev_bsdf_pdf, prev_bsdf_delta, si
    ):
        # Compute MIS weight for emitter sample from previous bounce
        ds = mi.DirectionSample3f(scene, si=si, ref=prev_si)

        mis = mis_weight(
            prev_bsdf_pdf, scene.pdf_emitter_direction(prev_si, ds, ~prev_bsdf_delta)
        )
    ...
    def sample_emitter(
        self, scene, sampler, throughput, bsdf_ctx, si, bsdf, active_next
    ):
